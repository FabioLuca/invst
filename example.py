...
#. **Fetch ticker data** from AlphaVantage API. For example purpose, two cases
   ...
   in sequence trying to fetch for a valid ticker (`GOOG`).
...
if __name__ == "__main__":
    ...
    #   and a request for GOOG for Google stocks OHLC.
    # --------------------------------------------------------------------------
    goog2 = DataAccess(
        ticker="GOOG2",
    ...
    file_export_tickers = f"Export_AlphaVantage_{today_string}.xlsx"
    file_export_tickers = Path.cwd().resolve() / "export" / file_export_tickers
    writer_ticker = pd.ExcelWriter(file_export_tickers, engine='xlsxwriter')
    goog_values.to_excel(writer_ticker, sheet_name='GOOG')
    writer_ticker.save()
